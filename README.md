...
세 단계로 나뉩니다. **① 처음 한 번 준비**하고, **② 회의 중에 자막을 보고**, **③ 회의가 끝난 뒤 회의록을 만듭니다.** ②와 ③은 서로 다른 도구가 맡습니다. 실시간 자막은 정확도가 낮아 기록으로 쓰지 않고, 회의록은 회의가 끝난 뒤에 따로 만듭니다.

`meeting` 을 인자 없이 실행하면 지금 무엇이 준비되었는지와 이 순서를 함께 보여 줍니다.

### ① 처음 한 번만

맥을 바꾸거나 새로 내려받았을 때만 다시 합니다. 자세한 내용은 아래 [설치](#설치)에 있습니다.
...
### ② 회의 중 — 상대방 말을 한국어 자막으로

| 상황 | 명령 |
| --- | --- |
| 외국 파트너와 온라인 회의. 상대 목소리가 스피커로 나옴 | `meeting live` |
| 대면 회의인데 상대가 영어로 말함 | `meeting live --mic` |
| 한국어 회의를 글로 받아 적기만 (번역 없음) | `meeting live --ko` |

회의를 시작할 때 켜 두면 됩니다. 스페이스로 멈추고 `q` 로 끝내며, 끝내면 자막이 `online/02-notes/` 에 남습니다.

### ③ 회의 후 — 회의록 남기기

**온라인 회의**는 Anarlog 앱이 마이크와 시스템 오디오를 함께 잡아 전사와 요약까지 처리합니다. 이 저장소의 스크립트는 쓰지 않고, 내보낸 노트를 `online/02-notes/` 에 두기만 하면 됩니다. 자세한 내용은 [online 항목](#online--온라인-회의)에 있습니다.

**대면 회의**는 아이폰으로 녹음한 뒤 세 단계를 거칩니다.

```bash
# 1) 아이폰 녹음을 맥으로 가져온다
meeting import-iphone

# 2) 전사와 화자분리를 거쳐 회의록을 만든다 (1시간 분량에 10~20분)
meeting transcribe

# 3) offline/03-notes/ 의 회의록을 열어 누가 누구인지 확인한 뒤 이름을 붙인다
meeting transcribe 주간회의.m4a --names SPEAKER_00=이우성 SPEAKER_01=김팀장
```

2단계까지 돌리면 화자가 `SPEAKER_00` 처럼 나옵니다. 누가 누구인지는 발화 내용을 봐야 알 수 있으므로, 회의록을 한 번 열어 확인한 다음 3단계로 이름을 붙입니다. 이때 전사는 다시 하지 않고 이름만 바꾸므로 몇 초면 끝납니다. 자세한 내용은 [offline 항목](#offline--현장-회의)에 있습니다.

### 정리하면

| 내가 원하는 것 | 쓰는 것 | 준비물 |
| --- | --- | --- |
| 회의 중 영어를 한국어로 보기 | `meeting live` | BlackHole, whisper.cpp, Ollama |
| 온라인 회의 회의록 | Anarlog 앱 | Anarlog |
| 대면 회의 회의록 | `meeting import-iphone` → `meeting transcribe` | whisperx, HuggingFace 토큰 |
