...
크게 두 가지를 합니다. **회의 중에 알아듣는 것**과 **회의가 끝난 뒤 회의록을 남기는 것**이며, 둘은 서로 다른 도구가 맡습니다. 실시간 자막은 정확도가 낮아 회의록으로 쓰지 않고, 회의록은 회의가 끝난 뒤에 따로 만듭니다.
...
회의를 시작할 때 켜 두면 됩니다. 스페이스로 멈추고 `q` 로 끝내며, 끝내면 자막이 `online/02-notes/` 에 남습니다. 처음 쓰기 전에 [설치 5단계](#5-실시간-자막을-쓴다면)를 한 번 마쳐야 합니다.
...
**대면 회의**는 아이폰으로 녹음한 뒤 두 단계를 거칩니다.

```bash
meeting import-iphone   # 아이폰 녹음을 offline/01-inbox 로 가져온다
meeting transcribe      # 전사와 화자분리를 거쳐 화자별 회의록을 만든다
```

`meeting transcribe` 는 1시간 분량에 10~20분이 걸리므로, 회의가 끝나고 걸어 두는 방식이 편합니다. 결과는 `offline/03-notes/` 에 화자별 마크다운으로 쌓이며, 처음에는 화자가 `SPEAKER_00` 처럼 나오므로 [4단계](#4-화자-이름-붙이기)에서 이름을 붙입니다. 자세한 내용은 [offline 항목](#offline--현장-회의)에 있습니다.
