...
│   ├── live             실시간 자막 (전사 + 번역)
│   ├── subtitle.swift   자막 창 (live 가 안에서 띄움)
...
| `meeting subtitle` | 자막 창입니다. `meeting live` 가 안에서 띄우므로 따로 부를 일은 드뭅니다. |
...
자막은 회의 화면 위에 떠 있는 창에 쌓입니다. 터미널처럼 흘러가 버리지 않아서, 놓친 대목은 위로 스크롤해 되짚어 볼 수 있습니다.

| 조작 | 하는 일 |
| --- | --- |
| 제목 막대를 끌기 | 창 옮기기 |
| 제목 막대를 두 번 누르기 | 크게 / 작게 (작아도 자막은 계속 흐릅니다) |
| 가장자리와 모서리를 끌기 | 크기 조절 (여덟 곳 모두) |
| `⋯` 단추 또는 본문 오른쪽 클릭 | 글자 크기, 투명도, 맨 위 고정 |
| `―` 단추 | 크게 / 작게 |
| `✕` 단추 | 끝내기 |

창의 자리와 크기, 글자 크기, 투명도는 닫을 때 기억해서 다음에 그대로 띄웁니다. 큰 크기와 작은 크기를 따로 기억하므로 두 상태를 오가도 흐트러지지 않습니다.

창을 쓰지 않고 터미널에 흘리려면 `--terminal` 을 붙입니다.

### 번역이 두 번 고쳐집니다

말이 끝나면 작은 모델이 1초 안쪽으로 먼저 옮겨 띄우고, 큰 모델이 앞뒤 맥락까지 보고 **같은 줄을 다시 고쳐 씁니다.**

```
지난 분기의 숫자들은 유망해 보입니다.     ← 먼저 뜨는 번역
지난 분기 실적이 좋게 나왔네요.           ← 맥락을 보고 다시 고친 번역
```

기다리는 동안 화면이 비지 않으면서도 결국 읽는 것은 다듬어진 문장입니다. 터미널에서는 이미 흘러간 줄을 고칠 수 없어 창에서만 되는 동작입니다. 작은 모델이 깔려 있지 않으면 한 단계로만 돌아갑니다.
